...
from utils import lonlat2meters, meters2lonlat, traj2cell_test_lr, traj2cell_test, draw_lr, draw
from dataset import test_data
import torch
from diffusion_model import MyDiscriminator
...
parser = argparse.ArgumentParser(description='Generate vector from SR')
    ...
    # ==========
    # ==========
parser.add_argument("--model_path", default='checkpoint/dma_bestmodel_MyG_3.pt',
...
def readfile(filename):
    ...
    for i in tqdm(range(dbnum)):
        if i < querynum:
            ...
            db.append(np.array(querydb.get('db/%s' % i), dtype="float64"))
    query = [item for item in query if item.size > 1]
...
def data_for_trjsr(file_a, file_b):
    ...
    with torch.no_grad():
        for traj in tqdm(data):
            ...
            # =============================
            # =============================
            test_traj = traj2cell_test(traj)
            test_traj_lr = traj2cell_test_lr(traj)
            
            test_img = ToTensor()(draw(test_traj, index = 0, mode = 'test'))
            test_img_lr = ToTensor()(draw_lr(test_traj_lr, index = 0, mode = 'test'))
            # print('test_img', test_img) 
            
            input = torch.unsqueeze(test_img, 0).cuda()
            input_lr = torch.unsqueeze(test_img_lr, 0).cuda()
            # print('input', input)
            # print('nonzero', torch.count_nonzero(input))
            # 乍一看大部分是0。用nonzero数一下里面还是有数字的
            
            
            t = netG.sample_timesteps(input.shape[0]).cuda()
            x_t, noise = netG.noise_images(input, t)
            predicted_noise = diffu_model(x = x_t, lr = input_lr, t = t)
            sr_img = sr_images(xt = x_t, lr = input_lr, model = diffu_model, epoch = 0, mode = 'test')
            # 学习到了predicted noise
            ...
            # =============================
            # =============================
            real_out, fake_out = netD(input, sr_img)
          
            traj_vec.append(fake_out)
...
def test_for_ssim(file_a, file_b, amount):
    ...
    with torch.no_grad():
        for traj in tqdm(data):
            test_traj = traj2cell_test(traj)
            test_traj_lr = traj2cell_test_lr(traj)
            test_img = ToTensor()(draw(test_traj))
            test_img_lr = ToTensor()(draw_lr(test_traj_lr))
            input = torch.unsqueeze(test_img, 0).cuda()
            input_lr = torch.unsqueeze(test_img_lr, 0).cuda()
            t = netG.sample_timesteps(input.shape[0]).cuda()
            x_t, noise = netG.noise_images(input, t)
...
if __name__ == '__main__':
    ...
    # ==========
    # ==========
    test_data(2900) # create dataset for evaluation
    # ==========
    # ==========
    
    # default: test_data(101000)
    ...
    for i in range(len(files_a)):
        ...
        #   data_for_trjsr里面用 训练好的模型生成vector
        #   sr_img = diffu_model(input)
        #   vec = netD(sr_img)
