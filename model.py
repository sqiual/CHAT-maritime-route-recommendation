...
#     def __init__(self, num, channels, length):
...
#         self.ma = SelfAttention(top_num + 1, c_in, db_in)
...
#             mse = mean_squared_error(db_org[0][:, :, :obs.shape[-1]].detach().clone().cpu().numpy().squeeze(axis = 0), db_org[i][:, :, :obs.shape[-1]].detach().clone().cpu().numpy().squeeze(axis = 0))
#             dist.append(mse)
#         top_sim_id = np.argsort(dist)
#         top_sim_id = top_sim_id[:min(self.top_num+1, len(db_org))]
...
# class TemporalBlock(nn.Module):
#     def __init__(self, top_num, n_inputs, db_in, n_outputs, kernel_size, stride, dilation, padding, dropout=0):
#         super(TemporalBlock, self).__init__()
#         self.conv1 = weight_norm(nn.Conv1d(n_inputs, n_outputs, kernel_size,
#                                            stride=stride, padding=padding, dilation=dilation))
#         self.chomp1 = Chomp1d(padding)
#         self.relu1 = nn.ELU()
#         self.dropout1 = nn.Dropout(dropout)
#         self.ma = SelfAttention(top_num+1, n_outputs, db_in)
#         self.conv2 = weight_norm(nn.Conv1d(n_outputs, n_outputs, kernel_size,
#                                            stride=stride, padding=padding, dilation=dilation))
#         self.chomp2 = Chomp1d(padding)
#         self.relu2 = nn.ELU()
...
#         res = x if self.downsample is None else self.downsample(x.float())
#         #return self.relu(out + res)
#         output = self.ma(res)
#         return output
...
# class TemporalConvNet(nn.Module):
#     def __init__(self, num, num_inputs, num_channels, length, kernel_size=2, dropout=0):
#         super(TemporalConvNet, self).__init__()
#         layers = []
#         num_levels = len(num_channels)
#         for i in range(num_levels):
...
#             in_channels = num_inputs if i == 0 else num_channels[i-1]
#             out_channels = num_channels[i]
#             layers += [TemporalBlock(num, in_channels, length, out_channels, kernel_size, stride=1, dilation=dilation_size,
#                                      padding=(kernel_size-1) * dilation_size, dropout=dropout)]
...
#         attention_value = self.ff_self(attention_value) + attention_value 
#         if final==True:
#             out = self.fc(attention_value.permute(1,2,0))  # [1, 480, 2] -> [480, 2, 1] / [1, 2, 2400]
#         else:
#             out = attention_value.permute(0,2,1)
#         return out
...
#         self.top_num = top_num
#         self.ma = SelfAttention(top_num + 1, c_in, db_in)
#         self.atcn_top = TemporalConvNet(num = top_num, num_inputs = c_in, num_channels = tcn_hidden, length = db_in)
#         self.lstm_db = nn.LSTM(c_in, c_in, batch_first = True, bidirectional = True)
...
#         top_sim_emb, top_sim_org = self.find_similar(obs, db_org) # top_sim_org [2, 5, 480]
#         #top_sim_org = torch.flatten(top_sim_org, 1).unsqueeze(dim = 0).float() # [1, 2, 2400]
#         top_sim_org = self.atcn_top(x = top_sim_org.permute(1,0,2)) # [5,2,480]
#         top_sim_org = self.ma(top_sim_org.float(), final = True)
#         top_sim_org = top_sim_org.permute(2,1,0)
