...
def reconstruct(model_pred_test, pred_test, obs_in, pred_in, origin, norm_max, norm_min):
    model_pred_test = model_pred_test * (norm_max - norm_min + 0.000001) + norm_min
    pred_test = pred_test * (norm_max - norm_min + 0.000001) + norm_min
...
def main(draw = False):
    ...
    path = 'checkpoint/CHAT_star6_antiTCN_1mon_top5_bestmodel_new2.pt'
    ...
    with torch.no_grad():
        ...
        for num, data in enumerate(test_bar):
            obs_test, pred_test, obs_org_test, pred_org_test, db_test, origin, norm_max, norm_min = data
            ...
            model_pred_test, pred_test = reconstruct(model_pred_test, pred_test, obs_in, pred_in, origin, norm_max.cuda(), norm_min.cuda())
    ...
    endtime = time.time()
    print("Inference time 1 days: ", endtime - starttime)
