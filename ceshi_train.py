...
parser.add_argument('--num_epoch', default=200, type=int, help='train epoch number')
...
parser.add_argument('--tcn_hidden', default=[3,5,2,2,2], type=list, help='# of step for diffusion')

parser.add_argument("--top", type=int, default=9, help="top n similar trajectory used for prediction")
parser.add_argument('--best_model', default='checkpoint/CHAT_star6_antiTCN_1mon_top9_bestmodel_new2.pt', help='name of the best model')

parser.add_argument("--pretrained", default='checkpoint/CHAT_star6_antiTCN_1mon_top9_bestmodel_new2.pt', type=str, help="path to pretrained model (default: none)")  
...
def save_checkpoint(state, is_best, modelname):
    filename = "checkpoint/CHAT_star6_antiTCN_1mon_top9_epoch_%d.pt" % (state["epoch"])
    if is_best:
...
def main():
    ...
    for epoch in range(pred_start_epoch, args.num_epoch):
        ...
        print("epoch:{} batch mean e_loss:{}".format(epoch, batch_val_loss))
